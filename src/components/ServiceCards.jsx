...
export function SolutionCard({ solution }) {
  ...
  return <StaggerItem as="link" direction="side" duration={key === 'financial' ? 0.85 : 0.65}
    ...
    <div className={styles.visual}>
      <ResponsiveImage image={solution.image} sizes="(max-width: 600px) 100vw, (max-width: 1100px) 50vw, 25vw" />
      <span className={styles.number}>{solution.number} / {solution.focus}</span>
    </div>
    <div className={styles.copy}>
      ...
      <p>{solution.summary || solution.description}</p>
      <div className={styles.outcome}><small>YOUR BUSINESS OUTCOME</small><strong>{solution.outcome}</strong></div>
      <ul className={styles.features}>{solution.features.map(feature => <li key={feature}>{feature}</li>)}</ul>
      <span className={styles.explore}>Explore services <span aria-hidden="true">↗</span></span>
    </div>
  </StaggerItem>;
}
