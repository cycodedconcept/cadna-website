...
import cardStyles from '../components/ConciergeCard.module.css';
import pageStyles from './PackagesPage.module.css';

const serviceIcons = ['search', 'growth', 'capital', 'technology', 'people', 'network'];

export function PackagesPage() {
  return <div><SiteNav active="packages" /><main id="main-content" tabIndex={-1}>
    <section className={pageStyles.hero} aria-labelledby="pricing-heading">
      <ResponsiveImage className={pageStyles.heroImage} image={images.listening} priority sizes="100vw" alt="" aria-hidden="true" />
      <FadeIn appear direction="fade" className={pageStyles.heroContent}>
        <span>PRICING & PACKAGES</span>
        <h1 id="pricing-heading">The right support. A quote built around you.</h1>
        <p>From business guidance to a strategic growth partnership, choose the level of support your business needs. We assess your requirements and provide a clear scope, timeline and tailored quotation.</p>
        <div className={`${styles.pills} ${pageStyles.heroLinks}`}><a href="#subscriptions">Annual subscriptions <span aria-hidden="true">↓</span></a><a href="#bundles">One-time services</a><a href="#custom">Get a tailored quote</a></div>
      </FadeIn>
    </section>
    ...
    ].map(([name, tag, description, request], index) => <article className={cardStyles.card} key={name}>
      <div className={cardStyles.header} aria-hidden="true"><span className={cardStyles.icon}><Icon name={serviceIcons[index]} size={30} /></span><span className={cardStyles.number}>{String(index + 1).padStart(2, '0')}</span></div>
      <small className={cardStyles.tag}>{tag}</small><h3>{name}</h3><p>{description}</p>
      <Link className={cardStyles.link} to={requestLink(request)}><span>One-Time Service Request<span className="srOnly"> — {name}</span></span><span className={cardStyles.arrow} aria-hidden="true"><Icon name="arrow" size={20} /></span></Link>
    </article>)}</div></section>
