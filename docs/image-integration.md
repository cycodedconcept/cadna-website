...
| CONCIERGE OFFICER.jpeg | 1024 × 1024 | Technology service cards |
...
| CADNA WEB IMAGES TECHNOLOGY CONCIERGE IMAGES.png | 1131 × 189 | Technology page hero; four supplied panels arranged in a two-by-two layout, retaining the original artwork and labels |
| CADNA WEB IMAGES EXPLANATORY IMAGES.png | 991 × 237 | Small diagram montage; workflow is implemented accessibly in React |
...
The largest shipped image is the 1536 px leadership WebP, about 172 KiB versus the 2.24 MiB source PNG (92.5% smaller). All exports, including alternative sizes, four small PNG icons and the social JPEG, total about 1.77 MiB; a page loads the sizes selected by the browser, not every variant.
...
`ServiceCards` is shared by Home and About; division pages use the same image data with an optional dedicated hero image. The Technology page uses the supplied platform montage, exported at 480 and 596 px without enlarging its individual panels. Its caption sits below the image so it does not obscure the interfaces. The blue brand accent and existing logo are retained, with navy surfaces, finance-specific dark tones and a subtle grid on the technology page. Keyboard focus, reduced motion and the skip-to-content link are supported. Previously global article, footer-small and request-form selectors are now scoped to prevent cross-page visual effects.
...
- The frontend brief form validates details and prepares an email or downloadable text file. It does not use an API or submit data automatically. Supporting documents must be attached separately in the visitor’s email app.
